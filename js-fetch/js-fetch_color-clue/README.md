...
1. Use the `fetch()` function to get data from the Color API. x
2. Use the `.json()` method to parse the body of the response and convert it into an object. x
3. Inspect the parsed object (e.g. by logging it to the console) and find the hex value and name of the closest named color. x
4. Call the `setColorToGuess()` function with the hex value and name of the closest named color. x
5. Guess colors! 🎉 x
